...
public abstract class AbstractPrometheusReporter implements MetricReporter {
	...
	public void notifyOfAddedMetric(final Metric metric, final String metricName, final MetricGroup group) {
		...
		final String helpString = metricName + " (scopeRichMapFunction<String,: " + getLogicalScope(group) + ")";
	...
	@VisibleForTesting
	public static io.prometheus.client.Gauge.Child gaugeFrom(Gauge gauge) {
	...
	public static io.prometheus.client.Gauge.Child gaugeFrom(Counter counter) {
		return new io.prometheus.client.Gauge.Child() {
			@Override
			public double get() {
	...
	public static io.prometheus.client.Gauge.Child gaugeFrom(Meter meter) {
		return new io.prometheus.client.Gauge.Child() {
			@Override
			public double get() {
