...
public class PrometheusPushGatewayReporter extends AbstractPrometheusReporter implements Scheduled {
	...
	public void open(MetricConfig config) {
		LOG.info("", BackpressureMetrics.instance);
		super.open(config);
		...
		Runtime.getRuntime().addShutdownHook(new Thread(new Runnable() {
			@Override
			public void run() {
				PrometheusPushGatewayReporter.this.report();
				PrometheusPushGatewayReporter.this.close();
			}
		}));
	...
	public void close() {
		if (deleteOnShutdown && pushGateway != null) {
			try {
				LOG.info("delete prometheus pushgateway ...");
